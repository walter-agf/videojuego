    ...
    ui(new Ui::multiplayer)
{
    ...
    bars.push_back(new grafica(this));
...
void multiplayer::actualizar()
{
    ...
    for ( int i = 0 ; i < bars.size(); i++){
        ...
        for (int j = 0; j < dis_j1.size(); j++){ // disparo uno de jefe
            ...
            if (dis_j1[j]->collidesWithItem(bars[i])){
        ...
        for (int u = 0; u < dis_j2.size(); u++){ // disparo dos del jefe
            ...
            if (dis_j2[u]->collidesWithItem(bars[i])){
            ...
            if (bars[i]->collidesWithItem(contras[a])){
                ...
                if (b->getPX() > 3840 && vida_j > 0 && disparo_jefe == 0){ //disparo recto
                    ...
                    if ( tipo_dis <= 5){
                        ...
                        dis_j1.back()->posy = 720 - bars[i]->getEsf()->getPY();
...
{
    elemento *b = bars[0]->getEsf(); //JUGADOR_1
    ...
    if (conti == true){
        ...
        if(event->key() == Qt::Key_W && bars[0]->getEsf()->getVY() <= 1 && bars[0]->getEsf()->getVY() >= -1){

            if (bars[0]->moment == 2  || bars[0]->moment == 0 ) {
                bars[0]->moment = 4;
                if (b->getVX()<=1){b->set_vel(b->getVX(),70,b->getPX(),b->getPY());}
                else {b->set_vel(b->getVX()+12 ,70,b->getPX(),b->getPY());}

            }
            else if (bars[0]->moment == 3 || bars[0]->moment == 1) {
                bars[0]->moment = 5;
                if (b->getVX()>=-1){b->set_vel(b->getVX(),70,b->getPX(),b->getPY());}
                else {b->set_vel(b->getVX()-12 ,70,b->getPX(),b->getPY());}
            }
        ...
        elemento *c = bars[1]->getEsf(); //JUGADOR_1

        if(event->key() == Qt::Key_L){

            bars[1]->moment = 2;

            c->set_vel(12,c->getVY(),c->getPX(),c->getPY());

            for (int a = 0;a < contras.size();a++) {

                if (bars[1]->collidesWithItem(contras[a]) && c->getPX() + c->getR() <= contras[a]->getposx()){
                    c->set_vel(-1*c->getE()*c->getVX(),c->getVY(),c->getPX(),c->getPY()+0.0171);
                }
            }
        }
        if(event->key() == Qt::Key_J){
            ...
            for (int a = 0;a < contras.size();a++) {
                ...
                if (bars[1]->collidesWithItem(contras[a]) && c->getPX() - c->getR() >= contras[a]->getposx() + contras[a]->getw()){
                    c->set_vel(-1*c->getE()*c->getVX(),c->getVY(),c->getPX(),c->getPY()+0.0171);
                }
            }
        ...
        if(event->key() == Qt::Key_I && bars[1]->getEsf()->getVY() <= 1 && bars[1]->getEsf()->getVY() >= -1){

            if (bars[1]->moment == 2  || bars[1]->moment == 0 ) {
                bars[1]->moment = 4;
                if (c->getVX()<=1){c->set_vel(c->getVX(),70,c->getPX(),c->getPY());}
                else {c->set_vel(c->getVX()+12 ,70,c->getPX(),c->getPY());}

            }
            else if (bars[1]->moment == 3 || bars[1]->moment == 1) {
                bars[1]->moment = 5;
                if (c->getVX()>=-1){c->set_vel(c->getVX(),70,c->getPX(),c->getPY());}
                else {c->set_vel(c->getVX()-12 ,70,c->getPX(),c->getPY());}
            }
        ...
        if (event->key() == Qt::Key_K){
            for (int a = 0;a < contras.size();a++) {
                if (bars[1]->collidesWithItem(contras[a]) && c->getPX() + 10 >= contras[a]->getposx() + contras[a]->getw()){
                    c->set_vel(0,c->getVY(),c->getPX(),c->getPY()+0.0171);
                }
            }
            if (bars[1]->moment == 4){
                bars[1]->moment = 8;
                c->set_vel(c->getVX(),c->getVY() - 16,c->getPX(),c->getPY());
            }
            else if (bars[1]->moment == 5){
                bars[1]->moment = 9;
                c->set_vel(c->getVX(),c->getVY() - 16,c->getPX(),c->getPY());
            }
            else if (bars[1]->moment == 2 && c->getVX() > 0){
                bars[1]->moment = 8;
                c->set_vel(c->getVX() + 16,c->getVY(),c->getPX(),c->getPY());
            }
            else if (bars[1]->moment == 3 && c->getVX() < 0){
                bars[1]->moment = 9;
                c->set_vel(c->getVX() - 16,c->getVY(),c->getPX(),c->getPY());
            }
        }
        //_____________Disparo____________________________________________
        if(event->key() == Qt::Key_P && con_cad == 0){
            //Prueba agregar un disparo_mago
            discab.push_back(new disparo_c);
            discab.back()->posx = bars[1]->getEsf()->getPX();
            discab.back()->posy = 720 - bars[1]->getEsf()->getPY();
            discab.back()->moment = bars[1]->moment;
            discab.back()->setPos(discab.back()->posx,discab.back()->posy);
            discab.back()->rango /= dificultad;
            scene->addItem(discab.back());
            con_cad = 100;
...
void multiplayer::on_actionPausa_triggered()
{
    if (timer->isActive()){
        timer->stop();
    }
...
void multiplayer::on_actionComo_Jugar_triggered()
{

}

void multiplayer::on_actionCargar_partida_triggered()
{
    if (jefe != NULL){
        scene->removeItem(jefe);
        jefe = NULL;
    ...
    if(k.is_open()){
        ...
        w_limit = 0;
        scene->setSceneRect(w_limit,0,1280,720);
        bars[0]->getEsf()->set_vel(0,0,50,300);
        bars[1]->getEsf()->set_vel(0,0,50,300);
        nivel(num);
  }
  else {
      QString val;
...
void multiplayer::on_actionGuardar_partida_triggered(){}
